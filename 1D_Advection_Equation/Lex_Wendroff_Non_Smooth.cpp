//LAX-WENDROFF SCHEME WITH EOC AND L2 NORM ERROR ESTIMATES
#include <iostream>
#include <cmath>
#include <fstream>
using namespace std;
...
// Lax-Wendroff scheme with fixed boundary conditions
void runLaxWendroff(double a, double L, double T, int M, int N, double** u, double* x, double* t) {
    ...
    for (int j = 0; j < N - 1; j++) {
        ...
        u_new[M-1] = 0.0;    // Right boundary
        
        // Apply Lax-Wendroff scheme for interior points
        for (int i = 1; i < M - 1; i++) {
            // Lax-Wendroff scheme:
            u_new[i] = u[i][j] 
                     - 0.5 * R * (u[i+1][j] - u[i-1][j])
                     + 0.5 * R_sq * (u[i+1][j] - 2.0 * u[i][j] + u[i-1][j]);
        }
...
int main() {
    ...
    // Run simulations
    runLaxWendroff(a, L, T, M1, N, u1, x1, t);
    runLaxWendroff(a, L, T, M2, N, u2, x2, t);
    runLaxWendroff(a, L, T, M3, N, u3, x3, t);
