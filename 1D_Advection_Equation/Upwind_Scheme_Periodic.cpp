...
// Analytical solution for fixed BCs: shift by at
double analyticalSolution(double x, double t, double a) {
    return sin(2.0 * M_PI * (x - a * t));
}

// FTBS scheme with fixed Dirichlet boundary conditions
void runUpwind(double a, double L, double T, int M, int N, double** u, double* x, double* t) {
    ...
    for (int j = 0; j < N - 1; j++) {
        u[0][j + 1] = u[0][j] - R * (u[0][j] - u[M - 2][j]);  // wrap around
        for (int i = 1; i < M - 1; i++) {
            u[i][j + 1] = u[i][j] - R * (u[i][j] - u[i - 1][j]);
        }
        u[M - 1][j + 1] = u[0][j + 1];  // to complete periodic wrap
...
int main() {
    double L = 10.0, T = 10.0, a = 01;
    int N = 1550;
    int M1 = 1400, M2 = 1800, M3 = 2600;

    // Allocate memory for M1
    double** u1 = new double*[M1]; for (int i = 0; i < M1; i++){    
         u1[i] = new double[N];
    }
    double* x1 = new double[M1];

    // Allocate memory for M2
    double** u2 = new double*[M2]; for (int i = 0; i < M2; i++){    
         u2[i] = new double[N];
    }
    double* x2 = new double[M2];

    // Allocate memory for M3
    double** u3 = new double*[M3]; for (int i = 0; i < M3; i++){    
         u3[i] = new double[N];
    }
    ...
    // Prepare 1D solution arrays
    double* u1_last = new double[M1];
    double* u2_last = new double[M2];
    for (int i = 0; i < M1; i++) u1_last[i] = u1[i][N - 1];
    for (int i = 0; i < M2; i++) u2_last[i] = u2[i][N - 1];

    // Write output file
    ofstream file("upwind_vs_analytical_all_meshes.csv");
    file << "x,u_numeric_M1,u_analytical,u_numeric_M2,u_analytical,u_numeric_M3,u_analytical\n";
    for (int i = 0; i < M3; i++) {
        double x_val = x3[i];
        double u_exact = analyticalSolution(x_val, T, a);
        ...
        file << x_val << "," << u_num_1 << "," << u_exact << "," << u_num_2 << "," << u_exact << "," << u_num_3 << "," << u_exact << "\n";
    }
    file.close();
    ...
    delete[] t; delete[] u1_last; delete[] u2_last;
